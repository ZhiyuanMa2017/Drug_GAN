...
def discriminator(x):
    with tf.variable_scope("discriminator"):
        ...
        x = tf.layers.dense(x, units=4 * 4 * 64, activation=leaky_relu)
...
def generator(z):
    with tf.variable_scope("generator"):
        z = tf.layers.dense(x, units=1024)
        z = tf.layers.batch_normalization(z, training=is_training)
        z = tf.nn.relu(z)
        z = tf.layers.dense(z, units=7 * 7 * 128)
        z = tf.layers.batch_normalization(z, training=is_training)
        z = tf.nn.relu(z)
        z = tf.reshape(z, [-1, 8, 8, 128])
        z = tf.layers.conv2d_transpose(z, filters=64, kernel_size=4, strides=2, padding='SAME')
        z = tf.layers.batch_normalization(z, training=is_training)
        z = tf.nn.relu(z)
...
with get_session() as sess:
    ...
    for h in range(10):
        fig = samples[h]
        plt.savefig('cifarfinal'+str(h)+'.jpg')