...
as possible for real inputs."""
    model = Sequential()
    model.add(Conv1D(input_shape=(116, 25),
                     nb_filter=25,
                     filter_length=4,
                     border_mode='same'))
    model.add(LeakyReLU())
...
def Generator(x_dash, y_dash, dropout=0.4, lr=0.00001, PATH="Gen.h5"):
    model = Sequential()
    # 6 features
    model.add(Dense(int(x_dash.shape[1]), activation="relu", input_shape=(6,)))
    #(None, 5800)
    model.add(Dense(int(116/4*25*8),activation="relu"))
    #(None, 29, 200)
    model.add(Reshape((int(116 / 4), int(25 * 8))))
    model.add(BatchNormalization(momentum=0.8))
    model.add(Dropout(dropout))
    model.add(UpSampling1D())
    #(None, 29, 200)
    model.add(Conv1D(int(25 * 8), kernel_size=4, padding="same"))
    model.add(Activation("relu"))
    model.add(BatchNormalization(momentum=0.8))
    model.add(Dropout(dropout))
    model.add(UpSampling1D())
    #(None, 116, 50)
    model.add(Conv1D(int(25 * 2), kernel_size=4, padding="same"))
    model.add(Activation("relu"))
    model.add(BatchNormalization(momentum=0.8))
    model.add(Dropout(dropout))
    #(None, 116, 25)
    model.add(Conv1D(int(25 * 1), kernel_size=4, padding="same"))
